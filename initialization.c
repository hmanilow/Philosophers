...
static int ft_fork_init(t_forks **forks_out, t_val *value)
{
	...
	forks = (t_forks *) malloc(sizeof(t_forks) * value->philo_amount);
	fork_mutex = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t) *
			value->philo_amount);
	if (!forks || !fork_mutex)
		return (1);
	i = 0;
	while (i < value->philo_amount)
	{
		forks[i].fork_id = i;
		forks[i].use = e_false;
		forks[i].phil_id = -1;
...
static int ft_philo_init(t_phil **phil_out, t_val *values, t_forks *forks)
{
	...
	i = -1; //???
	phil = (t_phil *) malloc(sizeof(t_phil) * values->philo_amount);
	if (!phil)
		return(1);
	current_time = get_current_time(); //dont work?
	while (++i < values->philo_amount)
	{
		...
		phil[i].left_fork = forks + (i + 1) % values->philo_amount;
...
int	ft_philo_forks_init(t_val *values, t_phil **phil_out, t_death *sbs_death)
{
	...
	*phil_out = phil;
	...
	sbs_death->num_phil = values->philo_amount;
	return (0);
}
